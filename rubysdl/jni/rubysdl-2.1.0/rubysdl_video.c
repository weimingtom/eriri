...
#if SDL_MAJOR_VERSION == 2
  if( SDL_SetColorKey(surface,SDL_TRUE,VALUE2COLOR(key,surface->format))
      < 0 ){
#else
...
static VALUE Surface_setAlpha(VALUE self,VALUE flag,VALUE alpha)
{
  rb_secure(4);
#if (SDL_MAJOR_VERSION == 1 && SDL_MINOR_VERSION == 3) || SDL_MAJOR_VERSION == 2
  /* see sdl-1.3 SDL_compat.c */
  if (NUM2UINT(flag) & SDL_SRCALPHA) {
		/* According to the docs, value is ignored for alpha surfaces */
		Uint8 value = NUM2INT(alpha);
		if (Get_SDL_Surface(self)->format->Amask) {
			value = 0xFF;
		}
		SDL_SetSurfaceAlphaMod(Get_SDL_Surface(self), value);
		SDL_SetSurfaceBlendMode(Get_SDL_Surface(self), SDL_BLENDMODE_BLEND);
  } else {
		SDL_SetSurfaceAlphaMod(Get_SDL_Surface(self), 0xFF);
		SDL_SetSurfaceBlendMode(Get_SDL_Surface(self), SDL_BLENDMODE_NONE);
  }
  SDL_SetSurfaceRLE(Get_SDL_Surface(self), (NUM2UINT(flag) & SDL_RLEACCEL));
#else
...
{
  Uint32 key = 0;
  SDL_GetColorKey(Get_SDL_Surface(self), &key);
  return UINT2NUM(key);
}
...
{
  Uint8 alpha = 0;
  SDL_GetSurfaceAlphaMod(Get_SDL_Surface(self), &alpha);
  return UINT2NUM(alpha);
}
#else
static VALUE PixelFormat_colorkey(VALUE self)
{
