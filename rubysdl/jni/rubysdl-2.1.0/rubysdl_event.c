...
//#define SDL_NUMEVENTS SDL_NUM_SCANCODES
#define SDL_NUMEVENTS 0xffff
#endif
...
/*see event/SDL_windowevents.c*/
  ...
  {
      VALUE obj = rb_obj_alloc(cActiveEvent);
      rb_iv_set(obj, "@gain", 0);
      return obj; 
  } 
  else if (event->window.event == SDL_WINDOWEVENT_RESIZED)
  {
      VALUE obj = rb_obj_alloc(cVideoResizeEvent);
      rb_iv_set(obj, "@w", INT2FIX(event->window.data1));
      rb_iv_set(obj, "@h", INT2FIX(event->window.data2));
      return obj;
  }
  VALUE obj = rb_obj_alloc(cActiveEvent);
  return obj;
...
static VALUE Event_s_push(VALUE class, VALUE event)
{
  ...
  if(eventClass == cActiveEvent){
    ...
    e.window.event = rb_iv_get(event, "@gain") != 0 ? SDL_WINDOWEVENT_ENTER : SDL_WINDOWEVENT_LEAVE;
	e.window.data1 = 0;
	e.window.data2 = 0;
#else
...
#if SDL_MAJOR_VERSION == 2
    e.key.keysym.scancode   = NUM2UINT( rb_iv_get(event, "@unicode") );
#else
    ...
    e.window.data1 = NUM2INT(rb_iv_get(event, "@w"));
    e.window.data2 = NUM2INT(rb_iv_get(event, "@h"));
#else
...
#if SDL_MAJOR_VERSION == 2
  event_creators[SDL_WINDOWEVENT] = createActiveEvent;
#else
