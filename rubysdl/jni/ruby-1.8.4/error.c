...
#ifdef ANDROID
    __android_log_print(ANDROID_LOG_ERROR, "ruby_error.c", "[%s:%d %s]%s", __FILE__, __LINE__, __FUNCTION__, buf);
#endif
    if (fwrite(buf, 1, len, out) == len ||
	fwrite(buf, 1, len, (out = stdout)) == len) {
#ifdef ANDROID
    __android_log_print(ANDROID_LOG_ERROR, "ruby_error.c", "[%s:%d %s][BUG] ", __FILE__, __LINE__, __FUNCTION__);
#endif
    fputs("[BUG] ", out);
    va_init_list(args, fmt);
#ifdef ANDROID
    __android_log_print(ANDROID_LOG_ERROR, "ruby_error.c", fmt, args);
#endif
	vfprintf(out, fmt, args);
	va_end(args);
#ifdef ANDROID
    __android_log_print(ANDROID_LOG_ERROR, "ruby_error.c", "[%s:%d %s]ruby %s (%s) [%s]\n",
    	__FILE__, __LINE__, __FUNCTION__, ruby_version, ruby_release_date, ruby_platform);
#endif
    fprintf(out, "\nruby %s (%s) [%s]\n\n",
