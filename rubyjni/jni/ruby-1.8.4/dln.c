...
{
#ifdef ANDROID
    __android_log_print(ANDROID_LOG_ERROR, "ruby_dln.c", "[%s:%d %s]   %s", __FILE__, __LINE__, __FUNCTION__, key);
#endif
    fprintf(stderr, "  %s\n", key);
    return ST_CONTINUE;
}
...
{
#ifdef ANDROID
    __android_log_print(ANDROID_LOG_ERROR, "ruby_dln.c", "[%s:%d %s] Undefined symbols:", __FILE__, __LINE__, __FUNCTION__);
#endif
...
#ifdef ANDROID
    __android_log_print(ANDROID_LOG_ERROR, "ruby_dln.c", "[%s:%d %s]dln: Calling undefined function", __FILE__, __LINE__, __FUNCTION__);
#endif
    ...
    int len;
{
    ...
    if (len < strlen(first)) {
#ifdef ANDROID
    __android_log_print(ANDROID_LOG_ERROR, "ruby_dln.c", "[%s:%d %s]PATH length too long: %s", __FILE__, __LINE__, __FUNCTION__, first);
#endif
...
# endif
	...
	if ((fspace -= i) < 0) {
	  toolong:
#ifdef ANDROID
	  	__android_log_print(ANDROID_LOG_ERROR, "ruby_dln.c", "[%s:%d %s]openpath: pathname too long (ignored)", __FILE__, __LINE__, __FUNCTION__);
		*bp = '\0';
		__android_log_print(ANDROID_LOG_ERROR, "ruby_dln.c", "[%s:%d %s]\tDirectory \"%s\"", __FILE__, __LINE__, __FUNCTION__, fbuf);
		__android_log_print(ANDROID_LOG_ERROR, "ruby_dln.c", "[%s:%d %s]\tFile \"%s\"", __FILE__, __LINE__, __FUNCTION__, fname);
#endif
...
#ifdef ANDROID
	if (ruby_jni_callback_isfileexist(fbuf)) {
		return fbuf;
	}
#endif
#ifndef __MACOS__
...
#ifdef ANDROID
			__android_log_print(ANDROID_LOG_ERROR, "ruby_dln.c", "[%s:%d %s]openpath: pathname too long (ignored)", __FILE__, __LINE__, __FUNCTION__);
			__android_log_print(ANDROID_LOG_ERROR, "ruby_dln.c", "[%s:%d %s]\tDirectory \"%.*s\"", __FILE__, __LINE__, __FUNCTION__, (int) (bp - fbuf), fbuf);
			__android_log_print(ANDROID_LOG_ERROR, "ruby_dln.c", "[%s:%d %s]\tFile \"%s%s\"", __FILE__, __LINE__, __FUNCTION__, fname, extension[j]);
#endif
		    fprintf(stderr, "openpath: pathname too long (ignored)\n");
		    fprintf(stderr, "\tDirectory \"%.*s\"\n", (int) (bp - fbuf), fbuf);
