...
#ifdef ANDROID
	if (!NIL_P(str)) {
    	__android_log_print(ANDROID_LOG_INFO, "ruby_io.c", "[%s:%d %s]%s", __FILE__, __LINE__, __FUNCTION__, RSTRING(str)->ptr);
    	fprintf(stdout, "io_write : %s\n", RSTRING(str)->ptr);
	}
#endif
...
retry:
  ...
  retry:
	...
	if (doexec) {
	    ...
	    rb_proc_exec(pname);
#ifdef ANDROID
	    __android_log_print(ANDROID_LOG_ERROR, "ruby_io.c", "[%s:%d %s]%s:%d: command not found: %s",
	    		__FILE__, __LINE__, __FUNCTION__, ruby_sourcefile, ruby_sourceline, pname);
#endif
...
#ifdef ANDROID    
    __android_log_print(ANDROID_LOG_INFO, "ruby_io.c", "[%s:%d %s]%s",
    	__FILE__, __LINE__, __FUNCTION__, RSTRING(rb_f_sprintf(argc, argv))->ptr);
...
{
#ifdef ANDROID
    //__android_log_print(ANDROID_LOG_ERROR, "ruby_io.c", "[%s:%d %s]%s",
    //	__FILE__, __LINE__, __FUNCTION__, RSTRING(rb_str_new(mesg, len))->ptr);
#endif
